...
function relatedName(value: readonly { full_name: string }[] | null | undefined, fallback: string) {
...
export async function listProjects(identity: WorkspaceIdentity): Promise<Project[]> {
  ...
  const result = await supabase.from("projects").select("id,code,name,product,description,status,target_date,owner:profiles!projects_owner_id_fkey(full_name)").eq("organization_id", identity.organizationId).is("deleted_at", null).order("updated_at", { ascending: false });
...
export async function listReports(identity: WorkspaceIdentity): Promise<Report[]> {
  ...
  const supabase = await createClient();
  const result = await supabase.from("reports").select("id,number,title,report_type,project_id,revision,status,confidentiality,updated_at,author:profiles!reports_author_id_fkey(full_name),reviewer:profiles!reports_reviewer_id_fkey(full_name),report_sections(id,title,content,source,sort_order),acceptance_criteria(id,measurement,operator,minimum,maximum,target,unit,result,outcome,override_reason)").eq("organization_id", identity.organizationId).is("deleted_at", null).order("updated_at", { ascending: false });
...
export async function listSuppliers(identity: WorkspaceIdentity): Promise<Supplier[]> {
  ...
  const supabase = await createClient();
  const result = await supabase.from("suppliers").select("id,code,name,supplier_type,country,risk,status,next_review,owner:profiles!suppliers_owner_id_fkey(full_name),supplier_documents(id,review_status)").eq("organization_id", identity.organizationId).is("deleted_at", null).order("name");
...
export async function listInspections(identity: WorkspaceIdentity): Promise<Inspection[]> {
  ...
  const supabase = await createClient();
  const result = await supabase.from("inspections").select("id,inspection_number,lot_id,component_id,inspected_at,sample_size,outcome,disposition,defects,inspector:profiles!inspections_inspector_id_fkey(full_name)").eq("organization_id", identity.organizationId).order("inspected_at", { ascending: false });
...
export async function listActivity(identity: WorkspaceIdentity): Promise<Activity[]> {
  ...
  const supabase = await createClient();
  const result = await supabase.from("activity_log").select("id,action,record_type,summary,created_at,actor:profiles!activity_log_actor_id_fkey(full_name)").eq("organization_id", identity.organizationId).order("created_at", { ascending: false }).limit(100);
