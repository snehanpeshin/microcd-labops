...
export function WorkspaceSetup() {
  ...
  async function submit(formData: FormData) {
    ...
    try {
      ...
      if (!token) throw new Error("Your session expired. Return to sign in.");
      ...
      if (!response.ok) throw new Error(result.error ?? "Workspace setup could not be completed.");
      await primeAuthServiceWorker(token);
      router.push("/app");
      router.refresh();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Workspace setup could not be completed.");
      setLoading(false);
    }
  }
