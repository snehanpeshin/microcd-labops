...
type DraftResponse = { suggestion:string; promptVersion:string; model:string; generatedAt:string; dlp?:{redacted:boolean;findings:{category:string;severity:string;count:number}[]}; error?:string };
...
export function AiReportAssistant({ reportId, projectId, objective, evidence, criteriaSummary, configured }: { reportId:string; projectId:string; objective:string; evidence:string; criteriaSummary:string; configured:boolean }) {
  ...
  const [metadata,setMetadata]=useState<Pick<DraftResponse,"promptVersion"|"model"|"generatedAt"|"dlp">|null>(null);
  ...
  async function generate(){
    ...
    try{
      const response=await fetch("/api/ai/draft",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({reportId,projectId,section,objective,evidence,criteriaSummary,confirmedTransfer})});
  ...
  return <div id="ai-assistant"><Card><CardHeader title="AI report assistant" description="Draft from this report’s evidence; you remain the author and decision-maker." action={<span className="grid size-9 place-items-center rounded-lg bg-violet-50 text-violet-700"><Sparkles size={18}/></span>}/><CardContent className="space-y-4">
    ...
    <div className="rounded-lg border border-violet-100 bg-violet-50/60 p-3 text-xs leading-5 text-violet-950"><strong>Evidence boundary</strong><p className="mt-1">The assistant receives a DLP-sanitized objective, report sections, and criteria summary. Credentials are blocked and common identifiers are redacted. It cannot change calculations, approve the report, or sign a record.</p></div>
    <label className="flex items-start gap-3 rounded-lg border border-slate-200 p-3 text-xs leading-5"><input className="mt-0.5 size-4 min-h-0" type="checkbox" checked={confirmedTransfer} onChange={(event)=>setConfirmedTransfer(event.target.checked)}/><span>I reviewed these fields and authorize sending their sanitized text to the configured AI service for this request. Do not include patient data, credentials, or confidential IP.</span></label>
    ...
    <Button type="button" className="w-full" onClick={generate} disabled={pending||!configured||!confirmedTransfer}>{pending?"Working…":"Generate evidence-grounded suggestion"}</Button>
    {metadata?.dlp?.redacted?<p role="status" className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900">Sensitive identifiers were redacted before transmission. Review the generated text for placeholders.</p>:null}
