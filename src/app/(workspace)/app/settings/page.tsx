...
export default async function SettingsPage() {
  ...
  if (!identity.demo) {
    ...
    const [memberResult, subscriptionResult] = await Promise.all([
      supabase.from("organization_members").select("user_id,role,profiles(full_name)").eq("organization_id", identity.organizationId).eq("status", "active"),
      supabase.from("subscriptions").select("plan,status").eq("organization_id", identity.organizationId).maybeSingle(),
    ]);
    if (memberResult.data) members = memberResult.data.map((member) => ({ id: member.user_id, name: member.profiles?.[0]?.full_name ?? "Member", email: "Email hidden", role: member.role }));
