...
export async function POST(request: Request) {
  ...
  const model = process.env.OPENAI_MODEL ?? "gpt-5-mini"; const promptVersion = "engineering-section-v2"; const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
  try {
    const response = await client.responses.create({ model, input: [{ role: "system", content: "Draft a concise engineering report suggestion using only supplied facts. Never invent measurements, criteria, outcomes, standards, approvals, identities, or conclusions. State 'not provided' for missing information. Separate observation from interpretation. The user must review and explicitly accept the suggestion. Return plain text only." }, { role: "user", content: `Section: ${parsed.data.section}\nObjective:\n${parsed.data.objective}\nEvidence:\n${parsed.data.evidence || "Not provided"}\nCriteria summary:\n${parsed.data.criteriaSummary || "Not provided"}` }] });
